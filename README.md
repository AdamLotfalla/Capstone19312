...
```
8. Find using ```Ctrl + F``` the lines with this comment: ```//uncomment if you want an upper limit for people count```, and optionally uncomment the whole line to put an upper limit to people count (there are 3 lines in total)
## Running the code
